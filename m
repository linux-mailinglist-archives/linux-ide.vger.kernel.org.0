...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 898EF33F0A8
	for <lists+linux-ide@lfdr.de>; Wed, 17 Mar 2021 13:46:44 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229809AbhCQMqL (ORCPT <rfc822;lists+linux-ide@lfdr.de>);
        Wed, 17 Mar 2021 08:46:11 -0400
Received: from mail.kernel.org ([198.145.29.99]:33200 "EHLO mail.kernel.org"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S229698AbhCQMpi (ORCPT <rfc822;linux-ide@vger.kernel.org>);
        Wed, 17 Mar 2021 08:45:38 -0400
Received: by mail.kernel.org (Postfix) with ESMTPSA id 3C94964F26;
        Wed, 17 Mar 2021 12:45:37 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1615985137;
        bh=Bt/qjPJKdNB0BagZn/7qOmatMPE++3lknPRvV0bn078=;
        h=Date:From:To:Cc:Subject:References:In-Reply-To:From;
        b=
Received: by pali.im (Postfix)
        id 82AE08A9; Wed, 17 Mar 2021 13:45:34 +0100 (CET)
Date:   Wed, 17 Mar 2021 13:45:34 +0100
...
Subject: Re: Kernel panic caused by plugging in a sata cable on a a
 minipcie-sata board
Message-ID: <20210317124534.qcikawlm33zy6xzu@pali>
References: <cbbb2496501fed013ccbeba524e8d573@posteo.de>
 ...
 <131bcfe9750061bf57a6b769212446d7@posteo.de>
MIME-Version: 1.0
...
Marek found a issue in that Asmedia SATA controller and sent patch
which should workaround it:

https://lore.kernel.org/linux-pci/20210317115924.31885-1-kabel@kernel.org/T/#u

Issue happen only with PCIe controllers which support 512 byte long
packets. Seems that this SATA controller presents that is capable of 512
byte long packets, but when it is enabled then controller / system
crashes.

And based on our testing, it is rare to have machine with PCIe which
support 512 byte long packets. None of our x86 Intel machine support it.

So I guess you would not be able to reproduce this issue on your x86
device and probably also not on low-end ARM device too.

You can check maximal size supported by PCIe controller by looking at
line "DevCap: MaxPayload 512 bytes" in lspci output for device named
"PCI bridge".

My Intel based laptops have only "DevCap: MaxPayload 128 bytes".

We have only two platforms (Marvell A3720 and CN9130) with 512 bytes,
which are affected and patch for both of them fixes support for that
Asmedia SATA controller.

On Espressobin (A3720 based) that issue appeared in kernel version which
introduced support for PCIe Root bridge which allows kernel to configure
MaxPayload size and therefore enable usage of 512 bytes.

On Tuesday 16 March 2021 23:36:00 Rötti wrote:
> Hello Pali,
> 
> I'll check that. That may take a few days because I'll have to organize a
> x86 device capable of using this controller.
